...
export default function App() {
  ...
  const assetByCat = useMemo(()=>{
    const m={};
    fAssets.forEach(a=>{m[a.category]=(m[a.category]||0)+Number(a.amount);});
    return Object.entries(m).map(([name,value])=>({name,value})).sort((a,b)=>b.value-a.value);
  },[fAssets]);

  // Liability breakdown by category
  const liabByCat = useMemo(()=>{
    const m={};
    fLiabs.forEach(a=>{m[a.category]=(m[a.category]||0)+Number(a.amount);});
    return Object.entries(m).map(([name,value])=>({name,value})).sort((a,b)=>b.value-a.value);
  },[fLiabs]);
      ...
      <div style={{position:"relative",zIndex:1}}>
        ...
        {tab==="balance"&&(
          <div style={{padding:"0 14px 100px"}}>
            ...
            {assetByCat.length>0&&(
              <div className="card">
                <div style={{fontWeight:700,marginBottom:14,fontSize:15}}>資產分佈 🥧</div>
                <ResponsiveContainer width="100%" height={200}>
                  <PieChart>
                    <Pie data={assetByCat} cx="50%" cy="50%" outerRadius={75} dataKey="value" label={({name,percent})=>`${name} ${(percent*100).toFixed(0)}%`} labelLine={false} fontSize={11}>
                      {assetByCat.map((_,i)=><Cell key={i} fill={COLORS[i%COLORS.length]}/>)}
                    </Pie>
                    <Tooltip formatter={v=>`$${fmt(v)}`}/>
                  </PieChart>
                </ResponsiveContainer>
                <div style={{display:"flex",flexWrap:"wrap",gap:8,marginTop:4}}>
                  {assetByCat.map((e,i)=>(
                    <div key={e.name} style={{display:"flex",alignItems:"center",gap:5,fontSize:12}}>
                      <div style={{width:9,height:9,borderRadius:"50%",background:COLORS[i%COLORS.length]}}/>
                      {e.name}<span style={{color:"#bbb",marginLeft:3}}>${fmt(e.value)}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {liabByCat.length>0&&(
              <div className="card">
                <div style={{fontWeight:700,marginBottom:14,fontSize:15}}>負債分佈 🥧</div>
                <ResponsiveContainer width="100%" height={200}>
                  <PieChart>
                    <Pie data={liabByCat} cx="50%" cy="50%" outerRadius={75} dataKey="value" label={({name,percent})=>`${name} ${(percent*100).toFixed(0)}%`} labelLine={false} fontSize={11}>
                      {liabByCat.map((_,i)=><Cell key={i} fill={COLORS[i%COLORS.length]}/>)}
                    </Pie>
                    <Tooltip formatter={v=>`$${fmt(v)}`}/>
                  </PieChart>
                </ResponsiveContainer>
